import { Component, ChangeDetectionStrategy, inject, signal, AfterViewInit, DestroyRef } from '@angular/core';
import { CommonModule, CurrencyPipe } from '@angular/common';
import { Router } from '@angular/router';
import { ButtonModule } from 'primeng/button';
import { CardComponent, LoadingComponent } from '@fiap-hackaton/shared-ui';
import { HubFacade, DashboardStats, ProductionFacade } from '@fiap-hackaton/dashboard-data-access';
import { Production, PRODUCTION_STATUS, calculatePlantingProgress, getDaysUntilHarvest } from '@fiap-hackaton/dashboard-domain';
import { AuthLoginFacade } from '@fiap-hackaton/auth-data-access';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
import { forkJoin } from 'rxjs';

interface DashboardCard {
  id: string;
  title: string;
  description: string;
  icon: string;
  route: string;
  gradient: string;
  iconColor: string;
}

@Component({
  selector: 'fiap-farms-hub',
  imports: [CommonModule, CardComponent, LoadingComponent, CurrencyPipe, ButtonModule],
  templateUrl: './hub.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class HubComponent implements AfterViewInit {
  protected readonly dashboardCards: DashboardCard[] = [
    {
      id: 'products',
      title: 'Produtos',
      description: 'Gerencie o catálogo completo de produtos da sua fazenda',
      icon: 'pi pi-box',
      route: '/dashboard/products',
      gradient: 'bg-gradient-to-br from-emerald-50 to-emerald-100',
      iconColor: 'text-emerald-600',
    },
    {
      id: 'plantings',
      title: 'Plantios',
      description: 'Registre e acompanhe todos os seus plantios em andamento',
      icon: 'pi pi-calendar',
      route: '/dashboard/plantings',
      gradient: 'bg-gradient-to-br from-green-50 to-green-100',
      iconColor: 'text-green-600',
    },
    {
      id: 'sales',
      title: 'Vendas',
      description: 'Registre e acompanhe todas as suas vendas e receitas',
      icon: 'pi pi-shopping-cart',
      route: '/dashboard/sales',
      gradient: 'bg-gradient-to-br from-blue-50 to-blue-100',
      iconColor: 'text-blue-600',
    },
  ];

  protected stats = signal<DashboardStats>({
    totalProducts: 0,
    totalSales: 0,
    monthlyRevenue: 0,
    recentSalesCount: 0,
  });

  protected activePlantings = signal<Production[]>([]);
  protected isLoadingStats = signal(false);
  protected isLoadingPlantings = signal(false);

  private readonly hubFacade = inject(HubFacade);
  private readonly productionFacade = inject(ProductionFacade);
  private readonly authFacade = inject(AuthLoginFacade);
  private readonly router = inject(Router);
  private readonly destroyRef = inject(DestroyRef);

  ngAfterViewInit(): void {
    setTimeout(() => this.loadDashboardData(), 0);
  }

  protected navigateTo(path: string): void {
    this.router.navigate([path]);
  }

  protected refreshStats(): void {
    this.loadDashboardData();
  }

  protected getPlantingProgress(planting: Production): number {
    return calculatePlantingProgress(planting);
  }

  protected getDaysRemaining(planting: Production): string {
    const days = getDaysUntilHarvest(planting);
    if (days < 0) return 'Pronto para colheita!';
    if (days === 0) return 'Colheita hoje!';
    if (days === 1) return '1 dia restante';
    return `${days} dias restantes`;
  }

  protected formatDate(timestamp: any): string {
    return timestamp.toDate().toLocaleDateString('pt-BR');
  }

  private loadDashboardData(): void {
    this.isLoadingStats.set(true);
    this.isLoadingPlantings.set(true);

    const currentUser = this.authFacade.currentUser();

    if (!currentUser?.id) {
      this.isLoadingStats.set(false);
      this.isLoadingPlantings.set(false);
      return;
    }

    forkJoin({
      stats: this.hubFacade.getDashboardStats(currentUser.id),
      plantings: this.productionFacade.getByStatus(currentUser.id, PRODUCTION_STATUS.IN_PRODUCTION)
    })
    .pipe(takeUntilDestroyed(this.destroyRef))
    .subscribe({
      next: ({ stats, plantings }) => {
        this.stats.set(stats);
        this.activePlantings.set(
          plantings
            .sort((a, b) => calculatePlantingProgress(b) - calculatePlantingProgress(a))
            .slice(0, 5)
        );
        this.isLoadingStats.set(false);
        this.isLoadingPlantings.set(false);
      },
      error: () => {
        this.isLoadingStats.set(false);
        this.isLoadingPlantings.set(false);
      },
    });
  }
}
